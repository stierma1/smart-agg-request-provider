...
RequestProvider.prototype.addTo = function(app){
  ...
  this.client.on("invoke-rule", function(data){
    ...
    if(data.predicate === "request(Url, ContentType)" || data.predicate === "request(Url, ContentType, Transform)"){
      self.sendData(data.groundings, data.payload, data.rule)
        .catch(function(err){
          console.log(err);
...
RequestProvider.prototype.sendData = function(groundings, payload, ruleId){
    ...
    return request.postAsync({url:url, headers:{"content-type":contentType}, formData:writeData});
}
